...
describe('Client e2e test', () => {
  ...
  describe('new Client page', () => {
    ...
    it('should create an instance of Client', () => {
      cy.get(`[data-cy="nom"]`).type('quantifying cultivate Accounts').should('have.value', 'quantifying cultivate Accounts');

      cy.get(`[data-cy="postnom"]`).type('XSS').should('have.value', 'XSS');

      cy.get(`[data-cy="prenom"]`).type('calculating').should('have.value', 'calculating');

      cy.get(`[data-cy="numero"]`).type('Licensed Buckinghamshire Steel').should('have.value', 'Licensed Buckinghamshire Steel');

      cy.get(`[data-cy="email"]`).type('Golden.Kiehn53@hotmail.com').should('have.value', 'Golden.Kiehn53@hotmail.com');
